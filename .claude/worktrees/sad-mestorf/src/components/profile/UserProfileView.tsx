...
const PILL_ICON_COLOR = '#7a7268';

function PillIcon({ type }: { type: 'briefcase' | 'pin' | 'city' }) {
  if (type === 'briefcase') {
    return (
      <Svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke={PILL_ICON_COLOR} strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round">
...
export default function UserProfileView({
  ...
  const pills: { key: string; iconType: 'briefcase' | 'pin' | 'city'; label: string }[] = [
    ...(profile.work_type ? [{ key: 'work_type', iconType: 'briefcase' as const, label: profile.work_type }] : []),
    ...(profile.neighborhood ? [{ key: 'neighborhood', iconType: 'pin' as const, label: profile.neighborhood }] : []),
